    ...
    return TaskExecutionResult(
        ...
        result={
            "character": {
                ...
                "age": 16,
                "gender": "男性",
                "appearance": ["煤けたような黒髪", "深い緑色の瞳", "痩身で小柄な体格"],
                "personality": ["好奇心旺盛", "やや内向的", "努力家"],
                "magic_talent": ["一般的な魔法は平均以下", "古代魔法に対して特異な感応力を持つ"],
            }
        },
    )
...
async def test_calculate_metrics(reviewer, test_task, test_result):
    ...
    llm_response = await reviewer._generate_with_template(
        "reviewer/evaluate", {"task": test_task.model_dump(), "result": test_result.model_dump()}
    )
