...
async def run_all_tests():
    ...
    for dir_name in ["director_test_data", "planner_test_data", "worker_test_data", "reviewer_test_data"]:
    ...
    print("\n=== Director AI テスト実行中... ===")
    try:
        director_identical, director_result1, director_result2 = await test_director_integration()
        test_results["director"] = {
            "success": director_identical,
            "message": "同じ入力に対して同じ出力が生成されました" if director_identical else "出力に差異があります",
    ...
    print("\n=== Planner AI テスト実行中... ===")
    try:
        planner_identical, planner_result1, planner_result2 = await test_planner_task_planning()
        test_results["planner"] = {
            "success": planner_identical,
            "message": "同じ入力に対して同じ出力が生成されました" if planner_identical else "出力に差異があります",
    ...
    print("\n=== Worker AI テスト実行中... ===")
    try:
        worker_identical, worker_result1, worker_result2 = await test_worker_task_execution()
        test_results["worker"] = {
            "success": worker_identical,
            "message": "同じ入力に対して同じ出力が生成されました" if worker_identical else "出力に差異があります",
