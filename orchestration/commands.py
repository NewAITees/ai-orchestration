...
class PlanTaskCommand(Command):
    ...
    def __init__(self, session: 'Session', task_id: str, requirements: List[str] = None):
        """
        タスク計画コマンドを初期化
        Args:
            session: セッションオブジェクト
    ...
    def execute(self) -> Dict[str, Any]:
        ...
        planner: Optional['BasePlannerAI'] = self.session.get_component("planner")
        if not planner:
        ...
        main_task_model = TaskModel(id=self.task_id, title=f"Main Task {self.task_id}", description="Main task description")
        ...
        try:
            ...
            if isinstance(plan_result, dict) and "subtasks" in plan_result:
                ...
                for subtask_data in plan_result.get("subtasks", []):
                    try:
                        ...
                        print(f"Warning: Failed to validate/add subtask {subtask_data.get('id')}: {val_err}")
                print(f"[PlanTaskCommand] Added {len(created_subtasks)} subtasks to session {self.session.id}")
            ...
            return {"error": "Planner に plan_task メソッドが実装されていません", "status": "failure"}
...
class ExecuteTaskCommand(Command):
    ...
    def __init__(self, session: 'Session', task_id: str, context: Dict[str, Any] = None):
        """
        タスク実行コマンドを初期化
        Args:
            session: セッションオブジェクト
    ...
    def execute(self) -> Dict[str, Any]:
        ...
        worker: Optional['BaseWorkerAI'] = self.session.get_component("worker")
        if not worker:
        ...
        task_to_execute: Optional['SubTask'] = self.session.get_subtask(self.task_id)
        if not task_to_execute:
            print(f"Error: Subtask with ID {self.task_id} not found in session {self.session.id}")
            return {"error": f"指定されたサブタスクIDが見つかりません: {self.task_id}", "status": "failure"}
        ...
        try:
            ...
            execution_result: TaskExecutionResult = worker.execute_task(task=task_to_execute, context=self.context)
            ...
            return {"error": "Worker に execute_task メソッドが実装されていません", "status": "failure"}
        except Exception as e:
...
class CommandDispatcher:
    ...
    def __init__(self, session: 'Session'):
        ...
        print(f"CommandDispatcher initialized for session {session.id} with commands: {list(self.command_map.keys())}")
    ...
    def execute_command(self, command_type: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"[CommandDispatcher] Command {command_type} finished with status: {result.get('status')}")
            return result
