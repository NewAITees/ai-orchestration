...
class DirectorMode(enum.StrEnum):
    """Director AIの動作モード"""
...
class ReviewerMode(enum.StrEnum):
    """Reviewer AIの評価モード"""
...
class ProcessPhase(enum.StrEnum):
    """処理フェーズ"""
            ...
            "examples": [
                {
                    ...
                    "metrics": {"quality": 0.8, "completeness": 0.9, "relevance": 0.85},
                }
            ]
        },
    )
...
class OrchestratorRequest(TypedDict, total=False):
    ...
    model: ModelName | None  # 使用モデル
    session_id: SessionID | None  # セッションID
    creative_options: CreativeModeOptions | None  # 創作モード設定
    params: dict[str, Any] | None  # その他のパラメータ
            ...
            "examples": [
                {
                    ...
                    "metrics": {"readability": 0.85, "coherence": 0.92, "creativity": 0.78},
                }
            ]
        },
    )
...
class PlanningResult(BaseModel):
    ...
    dependencies: dict[str, list[str]] = Field(
        default_factory=dict, description="サブタスク間の依存関係"
    )
    estimated_duration: dict[str, float] = Field(
        default_factory=dict, description="各サブタスクの推定所要時間"
    )
    resource_requirements: dict[str, list[str]] = Field(
        default_factory=dict, description="各サブタスクのリソース要件"
    )
            ...
            "examples": [
                {
                    ...
                    "metrics": {"accuracy": 0.95, "completion_time": 120.5},
                    "improvements": [],
                }
            ]
        },
...
class BaseAIComponent(ABC):
    ...
    def _create_message(
        self, receiver: Component, message_type: MessageType, content: dict[str, Any]
    ...
    def _create_error_message(
        self, receiver: Component, error_message: str
    ) -> OrchestrationMessage:
        """
        エラーメッセージを作成する
        Args:
        ...
        Returns:
            作成されたエラーメッセージ
        """
        return self._create_message(receiver, MessageType.ERROR, {"error": error_message})
