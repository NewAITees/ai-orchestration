...
class WorkerAI(BaseAIComponent):
        ...
        try:
            if action == "execute_task":
                ...
                if not task:
                    return [
                        self._create_error_message(
                            message.sender, f"タスクが見つかりません: {task_id}"
                        )
                    ]
                ...
                result = loop.run_until_complete(
                    self.execute_task(
                        task, context=context, style_guide=style_guide, **execution_params
                    )
                )
        ...
        **kwargs,
        ...
        try:
            ...
            requirements_met = await self._validate_requirements(
                result_content,
                task.requirements if hasattr(task, "requirements") and task.requirements else [],
            )
            ...
            execution_time = datetime.now() - start_time
            error_msg = f"タスク実行中にエラーが発生しました: {e!s}"
    ...
    async def _gather_context_data(self, task: SubTask, context: dict[str, Any]) -> dict[str, Any]:
        """
        コンテキスト情報を収集
    ...
    async def _validate_requirements(
        self, result_content: str, requirements: list[str]
    ) -> list[str]:
        """
        要件の充足度を検証
        ...
        try:
            ...
            variables = {"result_content": result_content, "requirements": requirements}
    ...
    def _determine_task_type(self, task: SubTask) -> str:
        ...
        if any(
            keyword in title or keyword in description
            for keyword in ["コード", "プログラム", "実装", "code", "programming", "implementation"]
        ):
            return "coding"
...
class DefaultWorkerAI(WorkerAI):
    ...
    async def execute_task(
        self, task: SubTask, context: dict[str, Any] | None = None, **kwargs
    ) -> TaskExecutionResult:
        """
        タスクを実行
