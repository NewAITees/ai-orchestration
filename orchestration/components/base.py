...
class AIComponentProtocol(Protocol):
    ...
    def get_status(self, task_id: TaskID) -> TaskStatusModel | None:
        """タスクの状態を取得"""
        pass

    def update_status(
        self, task_id: TaskID, status: TaskStatus, metadata: dict[str, Any] | None = None
    ) -> None:
        """タスクの状態を更新"""
        pass
...
class BaseAIComponent(ABC):
    ...
    def __init__(self, session, llm_manager=None) -> None:
        """初期化"""
            ...
            if message.type == MessageType.COMMAND:
                ...
                return [
                    self._create_error_response(
                        message.sender, f"未対応のメッセージタイプ: {message.type}"
                    )
                ]
        except Exception as e:
            return [
                self._create_error_response(message.sender, f"処理中にエラーが発生しました: {e!s}")
            ]
        ...
        if query_type == "status":
            ...
            return [
                self._create_response(
                    message.sender, {"status": "unknown"}, "status_query_response"
                )
            ]
        else:
            return [
                self._create_error_response(message.sender, f"未対応のクエリタイプ: {query_type}")
            ]

    def _create_response(
        self, receiver: Component, content: dict[str, Any], action: str
    ) -> OrchestrationMessage:
        """応答メッセージを作成"""
...
class PlannerProtocol(AIComponentProtocol, Protocol):
    ...
    async def plan_task(
        self, task: TaskModel, requirements: list[str] | None = None
    ) -> PlanningResult:
        """タスクの実行計画を生成"""
        pass
...
class WorkerProtocol(AIComponentProtocol, Protocol):
    ...
    def execute_task(
        self, task: SubTask, context: dict[str, Any] | None = None
    ) -> TaskExecutionResult:
        """タスクを実行"""
        pass
