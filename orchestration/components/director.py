...
from ..core.session import Session, SessionStatus, SubTask
...
class IDirectorAI(Protocol):
    ...
    @abstractmethod
    def get_task_status(self, task_id: str) -> TaskStatusModel | None:
        """指定されたタスクの状態を取得する"""
        pass
...
class DirectorAI(BaseAIComponent):
    ...
    def _process_command(self, message: OrchestrationMessage) -> list[OrchestrationMessage]:
        ...
        try:
            if action == "start_process":
                ...
                if not task_id:
                    return [
                        self._create_error_response(message.sender, "task_id が指定されていません")
                    ]

                # プロセス開始
                loop = asyncio.get_event_loop()
                result = loop.run_until_complete(self.execute_process(task_id))
                ...
                if not results:
                    return [
                        self._create_error_response(message.sender, "results が指定されていません")
                    ]

                # 結果統合
                loop = asyncio.get_event_loop()
                integrated_result = loop.run_until_complete(self.integrate_results(results))
                ...
                return [
                    self._create_error_response(message.sender, f"未対応のアクション: {action}")
                ]
        except Exception as e:
            traceback.print_exc()
            return [
                self._create_error_response(
                    message.sender, f"コマンド処理中にエラーが発生しました: {e!s}"
                )
            ]
    ...
    async def execute_process(self, task_id: TaskID) -> None:
        ...
        try:
            ...
            if not task:
                # メインタスクが存在しない場合は新規作成
                task = TaskModel(
                    id=task_id, title=f"タスク {task_id}", description="タスク説明なし"
                )
            ...
            if not await planner.validate_plan(plan_result):
                ...
                print("[Director] Using fallback plan")
            ...
            for subtask_data in execution_order:
                ...
                if evaluator:
                    ...
                    if isinstance(evaluation, dict) and evaluation.get("score", 0) < 0.7:
                        print("[Director] Low evaluation score, requesting improvements")
            ...
            error_msg = f"プロセス実行中にエラーが発生しました: {e!s}"
    ...
    async def integrate_results(
        self, results: list[TaskExecutionResult | dict[str, Any]]
    ) -> dict[str, Any]:
        """
        実行結果を統合して最終結果を生成
        ...
        try:
            ...
            try:
                ...
                parsed_result = {"integrated_content": integration_content, "format": "text"}
            ...
            error_msg = f"結果統合中にエラーが発生しました: {e!s}"
            print(f"[Director] {error_msg}")
            traceback.print_exc()
    ...
    def _is_successful(self, result: TaskExecutionResult | dict[str, Any]) -> bool:
        """
        結果が成功かどうかを判定
    ...
    def _create_fallback_plan(self, task: TaskModel | SubTask) -> dict[str, Any]:
        """
        フォールバック計画を作成
    ...
    async def _reexecute_with_improvements(
        ...
        worker,
        subtask: SubTask,
        result: TaskExecutionResult | dict[str, Any],
        improvements: list[dict[str, Any]],
    ) -> TaskExecutionResult | dict[str, Any]:
        """
        改善提案に基づいてタスクを再実行
        ...
        # コンテキスト情報の準備
        context = {"previous_result": result, "improvements": improvement_descriptions}

        # タスクの再実行
        improved_result = await worker.execute_task(
            subtask, context=context, style_guide={"improvement_focus": True}
        )
