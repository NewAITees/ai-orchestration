...
from ..llm.llm_manager import BaseLLMManager, LLMManager
...
class ReviewerAI(BaseAIComponent):
    ...
    def __init__(
        self, session: Session, llm_manager: LLMManager, output_dir: str = "./output"
    ) -> None:
        """
        初期化
        Args:
    ...
    async def process_message(self, message: OrchestrationMessage) -> list[OrchestrationMessage]:
        ...
        if message.type != MessageType.COMMAND:
            return [
                self._create_error_message(
                    message.sender, f"サポートされていないメッセージタイプ: {message.type}"
                )
            ]
        ...
        try:
            ...
            if not handler:
                return [
                    self._create_error_message(
                        message.sender, f"サポートされていないアクション: {action}"
                    )
                ]
            ...
            logger.error(f"メッセージ処理中にエラーが発生: {e!s}", exc_info=True)
            return [
                self._create_error_message(
                    message.sender, f"メッセージ処理中にエラーが発生しました: {e!s}"
                )
            ]

    async def _handle_review_task(
        self, message: OrchestrationMessage
        ...
        if not task_data:
            return [
                self._create_error_message(
                    message.sender, "review_task コマンドには 'task' データが必要です"
                )
            ]
        ...
        try:
            ...
            return [
                self._create_message(
                    message.sender, MessageType.RESPONSE, {"review": review.model_dump()}
                )
            ]
        except Exception as e:
            logger.error(f"タスクレビュー中にエラーが発生: {e!s}", exc_info=True)
            return [
                self._create_error_message(
                    message.sender, f"タスクレビュー中にエラーが発生しました: {e!s}"
                )
            ]
    ...
    async def _handle_suggest_improvements(
        ...
        if not review_data:
            return [
                self._create_error_message(
                    message.sender, "suggest_improvements コマンドには 'review' データが必要です"
                )
            ]
        ...
        try:
            ...
            logger.error(f"改善提案生成中にエラーが発生: {e!s}", exc_info=True)
            return [
                self._create_error_message(
                    message.sender, f"改善提案生成中にエラーが発生しました: {e!s}"
                )
            ]

    async def _handle_evaluate_task(
        self, message: OrchestrationMessage
    ) -> list[OrchestrationMessage]:
        ...
        if not task_id or not output:
            return [
                self._create_error_message(
                    message.sender, "evaluate_task コマンドには 'task_id' と 'output' が必要です"
                )
            ]
        ...
        try:
            ...
            return [
                self._create_message(
                    message.sender, MessageType.RESPONSE, {"evaluation": evaluation.model_dump()}
                )
            ]
        except Exception as e:
            logger.error(f"タスク評価中にエラーが発生: {e!s}", exc_info=True)
            return [
                self._create_error_message(
                    message.sender, f"タスク評価中にエラーが発生しました: {e!s}"
                )
            ]

    async def review_task(
        self, task: SubTask, result: TaskExecutionResult | None = None
    ) -> ReviewResult:
        """
        タスクをレビューし、結果を返す
        Args:
        ...
        try:
            ...
            error_msg = f"タスクレビュー中にエラーが発生しました: {e!s}"
            logger.error(error_msg, exc_info=True)
            task.status = TaskStatus.FAILED
            raise

    async def evaluate_task(
        self, task: str | SubTask, output: str | None = None
    ) -> EvaluationResult:
        """
        タスクとその結果を評価する
        ...
        try:
            ...
            error_msg = f"タスク評価中にエラーが発生しました: {e!s}"
            logger.error(error_msg, exc_info=True)
            raise
    ...
    async def suggest_improvements(self, review: ReviewResult) -> list[dict[str, Any]]:
        ...
        try:
            ...
            error_msg = f"改善提案の生成中にエラーが発生しました: {e!s}"
            logger.error(error_msg, exc_info=True)
    ...
    def _create_evaluation_context(
        self, task: SubTask, result: TaskExecutionResult | None
    ) -> EvaluationContext:
        """評価コンテキストを作成"""
        result_content = ""
        if result:
        ...
        return EvaluationContext(
            ...
            metrics=EvaluationMetrics(
                quality=0.0, completeness=0.0, relevance=0.0, creativity=0.0, technical_accuracy=0.0
            ),
        )

    async def _calculate_metrics(
        self, task: SubTask, result: TaskExecutionResult | None, llm_response: str
    ) -> EvaluationMetrics:
        """評価メトリクスを計算"""
        try:
            ...
            logger.warning(f"メトリクス抽出に失敗: {e!s}", exc_info=True)
            return EvaluationMetrics(
                quality=0.8, completeness=0.8, relevance=0.8, creativity=0.8, technical_accuracy=0.8
            )
    ...
    async def _extract_suggestions(self, llm_response: str) -> list[str]:
        ...
        try:
            ...
            logger.warning(f"提案抽出に失敗: {e!s}", exc_info=True)
            return []
    ...
    async def _extract_feedback(self, llm_response: str) -> str:
        ...
        try:
            ...
            logger.warning(f"フィードバック抽出に失敗: {e!s}", exc_info=True)
    ...
    def _create_evaluation_prompt(self, task: SubTask, result: TaskExecutionResult | None) -> str:
        """評価プロンプトを生成"""
        return f"""
        Task ID: {task.id}
    ...
    def _evaluate_completeness(self, task: SubTask, result: TaskExecutionResult | None) -> float:
        """タスクの完了度を評価する"""
        if result is None:
            return 0.0
        return result.get("metrics", {}).get("completeness", 0.0)

    def _evaluate_quality(self, task: SubTask, result: TaskExecutionResult | None) -> float:
        """タスクの品質を評価する"""
        if result is None:
            return 0.0
    ...
    def _generate_feedback(
        ...
        if completeness >= 0.8:
            ...
            feedback_parts.append(
                "タスクは部分的に完了していますが、いくつかの要素が不足しています。"
            )
        else:
