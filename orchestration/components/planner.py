...
class PlannerAI(BaseAIComponent):
    ...
    def process_message(self, message: OrchestrationMessage) -> list[OrchestrationMessage]:
        ...
        try:
            if action == "plan_task":
                ...
                if not task_id:
                    return [
                        self._create_error_message(message.sender, "task_id が指定されていません")
                    ]
                ...
                return [
                    self._create_message(
                        message.sender, MessageType.RESPONSE, {"plan": plan_result}
                    )
                ]
            elif action == "validate_plan":
                plan = content.get("plan")
                if not plan:
                    return [self._create_error_message(message.sender, "plan が指定されていません")]
                ...
                return [self._create_error_message(message.sender, f"未対応のアクション: {action}")]
        except Exception as e:
            error_msg = f"コマンド処理中にエラーが発生しました: {e!s}"
            print(f"[Planner] {error_msg}")
            return [self._create_error_message(message.sender, error_msg)]

    async def plan_task(
        self, task_id: str, requirements: list[str] | None = None
    ) -> dict[str, Any]:
        """タスク計画作成"""
        ...
        try:
            ...
            result_content = await self.llm_manager.generate_with_template(
                template_id, variables, temperature=0.0
            )
            ...
            error_msg = f"タスク計画中にエラーが発生しました: {e!s}"
            print(f"[Planner] {error_msg}")
            return {"task_id": task_id, "subtasks": [], "dependencies": {}, "error": error_msg}

    async def validate_plan(self, plan: dict[str, Any]) -> dict[str, Any]:
        """計画の検証"""
        try:
            # テンプレート変数の準備
            variables = {"plan": plan, "session_id": self.session.id}
            ...
            error_msg = f"計画検証中にエラーが発生しました: {e!s}"
            print(f"[Planner] {error_msg}")
            return {
                "is_valid": False,
                "issues": [error_msg],
    ...
    def _determine_task_type(self, task: TaskModel | None) -> str:
        """タスクタイプを判定"""
        if not task:
            return "generic"
        ...
        if any(
            ...
            for keyword in ["創作", "小説", "物語", "creative", "story"]
        ):
            return "creative"

        if any(
            keyword in title or keyword in description
            for keyword in ["コード", "プログラム", "実装", "code", "programming"]
        ):
            return "coding"

        if any(
            keyword in title or keyword in description
            for keyword in ["分析", "調査", "研究", "analysis", "research"]
        ):
            return "analysis"
