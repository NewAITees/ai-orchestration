...
class Debugger:
    ...
    def __init__(self, debug_level: str = "info") -> None:
        """
        デバッガーの初期化
    ...
    def record_error(self, error_type: str, error_msg: str) -> None:
        ...
        error_record = {"timestamp": datetime.now().isoformat(), "message": error_msg}
        self.error_history[error_type].append(error_record)
        self.log("error", f"{error_type}: {error_msg}")

    def get_error_history(self, error_type: str | None = None) -> dict[str, list]:
        """
        エラー履歴を取得
    ...
    def clear_error_history(self, error_type: str | None = None) -> None:
        """
        エラー履歴をクリア
