...
from typing import Dict, List, Any, Optional, Union, Callable, AsyncGenerator, cast
...
class AgnoClient:
    ...
    async def run_query(self, query: str, images: Optional[List[Image]] = None, **kwargs) -> str:
        """
        直接エージェントにクエリを実行
        ...
        for attempt in range(max_retries):
            try:
                ...
                error_msg = f"Error running query (attempt {attempt+1}/{max_retries}): {str(e)}"
                self.debugger.record_error("run_error", error_msg)
                
                if ("loading model" in str(e).lower() or "timed out" in str(e).lower()) and attempt < max_retries - 1:
                    # モデルロード中の場合は少し待ってリトライ
                    self.debugger.log("info", f"Model is loading, waiting {retry_delay}s before retry...")
                    await asyncio.sleep(retry_delay)
                    retry_delay *= 2  # 指数バックオフ
                    continue
                    
                if attempt >= max_retries - 1:
                    return f"エラーが発生しました: {str(e)}。Ollamaサーバーが起動しているか、選択したモデルが正しくインストールされているか確認してください。"
    ...
    async def setup(self) -> None:
        ...
        try:
            ...
            self.debugger.record_error(
                "setup_error",
                f"Error during agent setup: {str(e)}"
            )
            raise
    ...
    async def process_query(
        ...
        **kwargs
        ...
        try:
            ...
            # 直接エージェントを使用
            return await self.run_query(query, images=agno_images if agno_images else None, **kwargs)
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(f"{self.base_url}/api/tags") as response:
                    if response.status == 200:
                        ...
                        self.debugger.record_error(
                            "model_error",
                            f"Failed to retrieve models: {response.status}"
                        )
